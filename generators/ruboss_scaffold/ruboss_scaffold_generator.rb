...
class RubossScaffoldGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      # If you want your Flexible Rails application to be wired for Cairngorm...
      if options[:cairngorm]
        
        # Attachment_fu configuration...
        if options[:attachment_fu]
          m.template 'cairngorm/attachment_fu/model.rb',      File.join('app/models', class_path, "#{file_name}.rb")

          m.template 'cairngorm/attachment_fu/controller.rb', File.join('app/controllers', class_path, "#{file_name.pluralize}_controller.rb")

          m.template 'cairngorm/attachment_fu/model.as.erb',
          ...
          m.template 'cairngorm/attachment_fu/component.mxml.erb',
          ...
          m.template 'cairngorm/attachment_fu/command.as.erb', File.join('app/flex', base_folder, 'commands', "#{@class_name}Command.as")
        ...
        # Restful_Authentication configuration...
          ...
          m.template 'cairngorm/restful_authentication/model.rb',
                      File.join('app/models', class_path, "#{file_name}.rb")

          if options[:include_activation]
            %w( mailer observer ).each do |model_type|
              m.template "cairngorm/restful_authentication/#{model_type}.rb", File.join('app/models', class_path, "#{file_name}_#{model_type}.rb")
            end
          end

          m.template 'cairngorm/restful_authentication/controller.rb',
                      File.join('app/controllers', controller_class_path, "#{controller_file_name}_controller.rb")

          m.template 'cairngorm/restful_authentication/model_controller.rb',
                      File.join('app/controllers', model_controller_class_path, "#{model_controller_file_name}_controller.rb")

          m.template 'cairngorm/restful_authentication/authenticated_system.rb',
                      File.join('lib', 'authenticated_system.rb')

          m.template 'cairngorm/restful_authentication/authenticated_test_helper.rb',
                      File.join('lib', 'authenticated_test_helper.rb')

          if @rspec
            m.template 'cairngorm/restful_authentication/functional_spec.rb',
                        File.join('spec/controllers', controller_class_path, "#{controller_file_name}_controller_spec.rb")
            m.template 'cairngorm/restful_authentication/model_functional_spec.rb',
                        File.join('spec/controllers', model_controller_class_path, "#{model_controller_file_name}_controller_spec.rb")
            m.template 'cairngorm/restful_authentication/unit_spec.rb',
                        File.join('spec/models', class_path, "#{file_name}_spec.rb")
            m.template 'cairngorm/restful_authentication/fixtures_authenticate.yml',
                        File.join('spec/fixtures', "#{table_name}.yml")
          else
            m.template 'cairngorm/restful_authentication/functional_test.rb',
                        File.join('test/functional', controller_class_path, "#{controller_file_name}_controller_test.rb")
            m.template 'cairngorm/restful_authentication/model_functional_test.rb',
                        File.join('test/functional', model_controller_class_path, "#{model_controller_file_name}_controller_test.rb")
            m.template 'cairngorm/restful_authentication/unit_test.rb',
          ...
          m.template 'cairngorm/restful_authentication/helper.rb',
                      File.join('app/helpers', controller_class_path, "#{controller_file_name}_helper.rb")

          m.template 'cairngorm/restful_authentication/model_helper.rb',
                      File.join('app/helpers', model_controller_class_path, "#{model_controller_file_name}_helper.rb")

          # Rails View templates
          m.directory File.join('app/views', controller_class_path, controller_file_name)
          m.directory File.join('app/views', model_controller_class_path, model_controller_file_name)
          m.template 'cairngorm/restful_authentication/login.html.erb',  File.join('app/views', controller_class_path, controller_file_name, "new.html.erb")
          m.template 'cairngorm/restful_authentication/signup.html.erb', File.join('app/views', model_controller_class_path, model_controller_file_name, "new.html.erb")

          # Flex Login and Account View templates
          m.directory File.join('app/flex', base_package, "components/generated/users")
          m.template 'cairngorm/restful_authentication/login.mxml.erb', File.join('app/flex', base_package, "components/generated/users", "LoginBox.mxml")
          m.template 'cairngorm/restful_authentication/signup.mxml.erb', File.join('app/flex', base_package, "components/generated/users", "SignupBox.mxml")
          m.template 'cairngorm/restful_authentication/account.mxml.erb', File.join('app/flex', base_package, "components/generated/users", "AccountBox.mxml")

          # Flex Login and Session Command templates
          m.template 'cairngorm/restful_authentication/command.as.erb', File.join('app/flex', base_package, "commands", "SessionsCommand.as")
          m.template 'cairngorm/restful_authentication/model_command.as.erb', File.join('app/flex', base_package, "commands", "#{@class_name}Command.as")

          m.template 'cairngorm/restful_authentication/model.as.erb',
            File.join("app", "flex", base_folder, "models", "#{@class_name}.as"), 
            :assigns => { :ruboss_controller_name => "#{file_name.pluralize}" }
          ...
          %w(PasswordConfirmationValidator.as ServerErrors.as ServerErrorValidator.as).each do |file|
            m.file "cairngorm/restful_authentication/validators/#{file}", "app/flex/com/pomodo/validators/#{file}"
          end

          if options[:include_activation]
            # Mailer templates
            %w( activation signup_notification ).each do |action|
              m.template "cairngorm/restful_authentication/#{action}.html.erb",
        ...
        # If no plugins are specified, but you still want Cairngorm, this is what you will get...
          ...
          # Ruboss templates.
          m.template 'cairngorm/ruboss_scaffold/model.as.erb',
            File.join("app", "flex", base_folder, "models", "#{@class_name}.as"), 
            :assigns => { :resource_controller_name => "#{file_name.pluralize}" }

          m.template 'cairngorm/ruboss_scaffold/component.mxml.erb',
            File.join("app", "flex", base_folder, "components", "generated", "#{@class_name}Box.mxml"), 
            :assigns => { :resource_controller_name => "#{file_name.pluralize}" }
          ...
          m.directory File.join('app/flex', base_folder, "commands")
          m.template 'cairngorm/ruboss_scaffold/command.as.erb', File.join('app/flex', base_folder, 'commands', "#{@class_name}Command.as")

          m.template 'cairngorm/ruboss_scaffold/controller.rb.erb', File.join("app/controllers", controller_class_path, 
          ...
          m.template 'cairngorm/ruboss_scaffold/model.rb.erb', File.join("app", "models", "#{file_name}.rb"), 
          ...
          unless options[:skip_migration]
            m.migration_template 'cairngorm/ruboss_scaffold/migration.rb.erb', 'db/migrate', :assigns => {
              :migration_name => "Create#{class_name.pluralize.gsub(/::/, '')}"
